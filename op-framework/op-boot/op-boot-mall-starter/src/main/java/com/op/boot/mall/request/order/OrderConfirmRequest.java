package com.op.boot.mall.request.order;

import com.op.boot.mall.client.MallAuthentication;
import com.op.boot.mall.constans.MallMethodConstants;
import com.op.boot.mall.constans.MallType;
import com.op.boot.mall.request.MallRequest;
import com.op.boot.mall.response.order.OrderConfirmResponse;

/**
 * 订单确认请求
 *
 * @param <P> 请求对象类型
 * @author cdrcool
 */
public class OrderConfirmRequest<P> extends MallRequest<P, OrderConfirmResponse> {

    public OrderConfirmRequest(MallType mallType, MallAuthentication authentication, P requestObj) {
        super(mallType, authentication, requestObj);
    }

    @Override
    public String getRequestMethod() {
        return MallMethodConstants.ORDER_CONFIRM;
    }

    @Override
    public Class<OrderConfirmResponse> getResponseClass() {
        return OrderConfirmResponse.class;
    }
}
