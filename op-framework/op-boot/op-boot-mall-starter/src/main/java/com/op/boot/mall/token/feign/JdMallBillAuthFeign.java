 ...
 * @author chengdr01
 */
@FeignClient(name = "jd-mall-bill-auth", url = "${mall.jd-bill.auth-url}", configuration = JdMallFeignConfig.class)
public interface JdMallBillAuthFeign {
     ...
     * @param sign      签名，生成规则如下：
     *                  1. 按照以下顺序将字符串拼接起来：client_secret+timestamp+client_id+username+password+grant_type+client_secret，其中：
    ...
    JdMallBaseResponse<JdMallTokenResponse> getAccessToken(@RequestParam("grant_type") String grantType,
    ...
    JdMallTokenResponse refreshToken(@RequestParam("refresh_token") String refreshToken,
                                     @RequestParam("client_id") String clientId,
